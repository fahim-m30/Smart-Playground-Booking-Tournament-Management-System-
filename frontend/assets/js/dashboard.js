...
async function loadNotifications() {
    ...
    try {
        ...
        list.innerHTML = notifications.length ? notifications.map((notification) => {
            ...
            return `<article class="notification-item${notification.readAt ? "" : " unread"}"><a class="notification-open" href="${escapeHTML(notification.link || "#")}" data-notification-id="${notification._id}"><strong>${escapeHTML(notification.title)}</strong><span>${escapeHTML(notification.message)}</span><time>${escapeHTML(when)}</time></a><button class="notification-delete" type="button" data-delete-notification-id="${notification._id}" aria-label="Delete notification" title="Delete notification"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M10 11v6m4-6v6M9 7l1-2h4l1 2m-8 0 1 13h8l1-13"/></svg></button></article>`;
