...
async function manageMatches(tournamentId) {
    try {
        ...
        const enhancedMatchRows = matches.length ? matches.map((match) => {
            ...
            const scheduleForm = isCompleted ? "" : `<form class="form-grid schedule-match" data-match-id="${match._id}"><input name="matchDate" type="date" value="${scheduleDate}" required><input name="startTime" type="time" value="${esc(scheduleStart)}" required><input name="endTime" type="time" value="${esc(scheduleEnd)}" required>${isCancelled ? '<input name="matchStatus" type="hidden" value="Scheduled">' : `<select name="matchStatus"><option ${match.matchStatus === "Scheduled" ? "selected" : ""}>Scheduled</option><option ${match.matchStatus === "Live" ? "selected" : ""}>Live</option></select>`}<button>${isCancelled ? "Confirm replay schedule" : "Save schedule"}</button></form>`;
            const cancelForm = !isCompleted && !isCancelled ? `<form class="form-grid cancel-match" data-match-id="${match._id}"><h4 class="full">Cancel & announce replay</h4><select name="cancellationReason" required><option value="">Cancellation reason</option><option>Weather</option><option>Unsafe playing conditions</option><option>Venue issue</option><option>Power outage</option><option>Equipment issue</option><option>Security or emergency</option><option>Official decision</option><option>Other</option></select><textarea class="full" name="cancellationDetails" maxlength="500" placeholder="Official notice for both teams (for example, heavy rain made the pitch unsafe)" required></textarea><input name="rescheduledDate" type="date" required><input name="rescheduledStartTime" type="time" required><input name="rescheduledEndTime" type="time" required><button class="full danger-action">Cancel match & notify teams</button></form>` : "";
            ...
            const result = !isCompleted && !isCancelled && match.matchStatus === "Live" && user.role === "playground-admin" ? `<form class="form-grid result-match" data-match-id="${match._id}"><label>${esc(match.teamA?.teamName || "Team A")} ${scoreLabel}<input name="teamAScore" type="number" min="0" value="${savedScoreA}" required></label><label>${esc(match.teamB?.teamName || "Team B")} ${scoreLabel}<input name="teamBScore" type="number" min="0" value="${savedScoreB}" required></label>${cricketWickets(match)}<p class="meta full">For cricket, enter runs and wickets as they change (for example, 350 runs and 8 wickets = 350/8). Publish the final result only after the match ends.</p><div class="full live-score-actions"><button class="alt" type="submit" data-result-action="live">Update live score</button><button type="submit" data-result-action="final">Publish final result</button></div></form>` : `<p class="meta">${isCompleted ? `Final score: ${match.teamAScore}${tournament.sportType === "Cricket" && Number.isFinite(Number(match.teamAWickets)) ? `/${match.teamAWickets}` : ""} - ${match.teamBScore}${tournament.sportType === "Cricket" && Number.isFinite(Number(match.teamBWickets)) ? `/${match.teamBWickets}` : ""}` : isCancelled ? "Result entry is locked until the replay schedule is confirmed." : match.matchStatus === "Scheduled" && user.role === "playground-admin" ? "Set the fixture status to Live before publishing its final result." : "Only the venue playground admin can publish the result."}</p>`;
        ...
        modal.innerHTML = `<div class="modal-box match-manager"><button class="close" type="button">Close</button><h2>${esc(tournament.name)} fixtures</h2><p class="meta">Cancellation requires an official reason and announced replay slot. Every round is locked until all matches in its previous round are completed.</p><div class="match-manager-list">${enhancedMatchRows}</div></div>`;
        document.body.append(modal);
        modal.querySelector(".close").onclick = () => modal.remove();
...
document.head.insertAdjacentHTML("beforeend", '<link rel="stylesheet" href="assets/css/tournament-centre.css"><link rel="stylesheet" href="assets/css/tournament-admin.css?v=20260901contrast5">');
