...
async function manageMatches(tournamentId) {
    try {
        ...
        const cricketWickets = (match) => tournament.sportType === "Cricket" ? `<input name="teamAWickets" type="number" min="0" max="10" placeholder="${esc(match.teamA?.teamName || "Team A")} wickets"><input name="teamBWickets" type="number" min="0" max="10" placeholder="${esc(match.teamB?.teamName || "Team B")} wickets">` : "";
        ...
        modal.querySelectorAll(".result-match").forEach((form) => form.onsubmit = async (event) => {
            event.preventDefault();
            const raw = Object.fromEntries(new FormData(form));
            try { await req(`/tournaments/matches/${form.dataset.matchId}/result`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ teamAScore: Number(raw.teamAScore), teamBScore: Number(raw.teamBScore), ...(tournament.sportType === "Cricket" ? { teamAWickets: Number(raw.teamAWickets), teamBWickets: Number(raw.teamBWickets) } : {}), matchStatus: "Completed" }) }); say("Match result published and standings updated."); modal.remove(); manageMatches(tournamentId); }
            catch (error) { say(error.message, true); }
        });
    } catch (error) { say(error.message, true); }
}
